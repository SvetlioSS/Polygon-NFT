...
describe('FxLimeGameItem', function () {
  ...
  it('Should successfully mint a token when called by owner', async function () {
    const mintResultsTx = await fxLimeGameItemContract
      .connect(owner)
      ['mint(address,string)'](addr1.address, 'some.dummy.url');
  ...
  it('Should throw an error when trying to mint a token when called by non-owners', async function () {
    expect(
      fxLimeGameItemContract.connect(addr1)['mint(address,string)'](addr1.address, 'some.dummy.url')
